...
import function_evaluate_policy_tokens as fep
...
def solve_singlegame(name_pa, data_parameter_dir=fb.data_parameter_dir, grid_version=fb.grid_version, result_dir=None, postfix='', gpu_device=None):
    ...
    [aiming_grid, prob_grid_normalscore_nt, prob_grid_singlescore_nt, prob_grid_doublescore_nt, prob_grid_triplescore_nt, prob_grid_bullscore_nt] = function_get_aiming_grid.load_aiming_grid(name_pa, data_parameter_dir=data_parameter_dir, grid_version='custom_tokens')
    [aiming_grid, prob_grid_normalscore_t, prob_grid_singlescore_t, prob_grid_doublescore_t, prob_grid_triplescore_t, prob_grid_bullscore_t] = function_get_aiming_grid.load_aiming_grid(name_pa, data_parameter_dir=data_parameter_dir, grid_version='custom_no_tokens')
...
def solve_dp_turn(aiming_grid, prob_grid_normalscore_nt, prob_grid_doublescore_nt, prob_grid_bullscore_nt, prob_grid_normalscore_t, prob_grid_doublescore_t, prob_grid_bullscore_t):
    ...
    for score_max in range(60):    
        ...
        prob_bust_dic_nt[score_max] = 1 - prob_notbust_dic_t[score_max]
    ...
    optimal_value_rt3_h = np.zeros((502,max_tokens+1))
    optimal_value_dic_h = {}
    optimal_action_index_dic_h= {}

    for t in range(0,max_tokens+1):
        ...
        for score_state in range(2, 502):
            ...
            if t==0:
                ...
                for round_index in range(iteration_round_limit):
                    ...
                    prob_turn_transit = fep.solve_turn_transit_probability_fast(score_state, state_action, prob_grid_normalscore_nt, prob_grid_doublescore_nt, prob_grid_bullscore_nt, prob_bust_dic_nt)        
                ...
                optimal_action_index_dic_h[t] = optimal_action_index_dic
                optimal_value_dic_h[t] = optimal_value_dic
                optimal_value_rt3_h[:,t] = optimal_value_rt3
                ## done:V(s,i=3/2/1,u)
                ...
                for round_index in range(iteration_round_limit):
                    ...
                    score_gained = 0
                    score_max_turn = min(score_state-2, 3*fb.maxhitscore)
                    prob_turn_transit = fep.solve_turn_transit_probability_fast(score_state, state_action, prob_grid_normalscore_t, prob_grid_doublescore_t, prob_grid_bullscore_t, prob_bust_dic_t)        
                    prob_turn_zeroscore = prob_turn_transit['bust'] + prob_turn_transit['score'][0]
                    new_value_rt3 = (1 + np.dot(prob_turn_transit['score'][1:], optimal_value_rt3[score_state-1:score_state-score_max_turn-1:-1])) / (1-prob_turn_zeroscore)
                    state_value_update[rt][score_gained] = new_value_rt3
                    optimal_value_rt3[score_state] = new_value_rt3
                    ...
                    for rt in [1,2,3]:            
                        ...
                        if (state_notbust_len > 0):
                            ...
                            ## transit to next throw in the same turn when rt=3,2
                            if (rt > 1):                    
                                num_turns_tensor = prob_normalscore_tensor_t.matmul(next_state_value_tensor)
                            ## transit to next turn when rt=1
                            else:
                                num_turns_tensor = 1 + prob_normalscore_tensor_t.matmul(next_state_value_tensor)
                        ...
                        if (state_notbust_len < this_throw_state_len):
                            ...
                            for score_gained in range(state_notbust_len, this_throw_state_len):
                                ...
                                if (rt > 1):
                                    if (score_remain == fb.score_DB):                        
                                        num_turns_array[:,score_gained_index] += prob_DB_t
                                    elif (score_remain <= 40 and score_remain%2==0):
                                        num_turns_array[:,score_gained_index] += prob_doublescore_dic_t[score_remain]
                                    else:
                                        pass
            
                                ## transit to bust
                                if (rt==3):
                                    num_turns_array[:,score_gained_index] += prob_bust_dic_t[score_max]
                                    ## solve an equation other than using the policy evaluation value (s,i=3,u=0)
                                    num_turns_array[:,score_gained_index] = num_turns_array[:,score_gained_index] / prob_notbust_dic_t[score_max] 
                                elif (rt==2):
                                    num_turns_array[:,score_gained_index] += prob_bust_dic_t[score_max]*(1+new_value_rt3)
                                else:
                                    num_turns_array[:,score_gained_index] += prob_bust_dic_t[score_max]*(new_value_rt3)  ## 1 turn is already counted before
    ...
    result_dic = {'optimal_value_dic':optimal_value_dic_h, 'optimal_action_index_dic':optimal_action_index_dic_h, 'optimal_value_rt3':optimal_value_rt3_h, 'prob_scorestate_transit':prob_scorestate_transit}
