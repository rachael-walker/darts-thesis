...
class Handicap:
    ...
    def __init__(self, results_file_name='./result/player10_turn_results.csv', epsilon_list = list(range(1,10))):

        self.df = pd.read_csv(results_file_name)
        if 'noturn' in results_file_name:
        ...
        self.spot_dart_optimal_lookup = self.init_spot_dart_optimal_handicap()
    ...
    def init_spot_dart_optimal_handicap(self):

        df_501_b0 = self.df.loc[(self.df.score==501) & (self.df.tokens==0)].copy()

        try: 
            df_501_b0 = df_501_b0.loc[df_501_b0.remaining_throws==3].copy()
        except: 
            pass 

        sd_handicap_vals = {}

        for s in range(len(self.epsilon_list)):

            sd_handicap_vals[self.epsilon_list[s]] = {}

            v501_s = df_501_b0[df_501_b0.epsilon==self.epsilon_list[s]].optimal_value.values[0]

            for w in range(len(self.epsilon_list)):
    ...
    def get_spot_dart_optimal_handicap(self,epsilon_strong,epsilon_weak):

        if epsilon_strong > epsilon_weak:
            print('Epsilon values are in the wrong order. Stronger player should be first.')
            return -1 

        spot_darts = self.spot_dart_optimal_lookup[epsilon_strong][epsilon_weak]
        #spot_darts = spot_darts_handicap.round()

        return spot_darts

    def get_dynamic_credits_handicap_full(self,epsilon_strong,epsilon_weak):
